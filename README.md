...
You can find the whole documentation of the code [here](https://samueled98.github.io/final_assignment/) (as requested in the first assignment of the RT2 course)
...
- The Move_Base server takes a non-negligible amount of time to notify the successful reaching of a point.  Here, for example, the robot was asked to reach the very point it was located.  ![countdown](/images/countdown.png)  It took slightly more than 3 minutes to the Move_Base server to notify the reached point.  During this time the countdown could reach zero and set the automatic canceling of the goal, resulting in a non completed operation (that actually was completed). To avoid this situation, it is, of course, possible to increase the time given to the robot to reach the target. This will, though, make the countdown useless since the program will either abort the operation for an unreachable target or actually reach it before it could go off.  What if the user wants to know if a point is actually reachable in a specific amount of time? The program should return the accomplished operation before the countdown expires and before the move_base server notify the "reached point" event. It is possible to do so by comparing the actual position of the robot to the desired one and if they are the same (maybe with a given tolerance) the goal is accomplished.  
- When the driving assistant is active, a particular situation may occur: the robot could find itself in a corner, where both the linear and the angular (both left and right) velocities are forced to zero. Since the robot cannot rotate on itself, it will necessarily drive backwards, potentially incurring in a collision (sensors provides view only in the front side).  
 This situation is solvable by:
   * Letting the robot rotate on itself if the only other choice it has is to blindly drive backwards.  
   * Setting different dangerous distances (where the robot must stop) for the frontal and the lateral sectors in order to reset the two velocities at different times.
